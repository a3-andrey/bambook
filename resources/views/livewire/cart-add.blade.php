<div class="cart__buttons @if(!$isProduct) ctg @endif">
    @if($qtu > 0)
        <div class="btn count-cart">
            <button wire:click.prevent="minus" class="minus-cart"></button>
            <div wire:loading  wire:target="plus, minus,updateCart" class="preloader-cart-product-add-btn  @if(!$isProduct) ipd @endif">
            ...
            <input onchange="@this.updateCart({{ $product->id }},this.value)"
                   value="{{ $qtu }}"
                   class="count"
                   type="text" >
            <button wire:click.prevent="plus"
                    class="plus-cart"></button>
        </div>
        @if($isProduct)
        <p class="add__item rotate">+</p>
        <p class="add__item">
            @if($isSale)
                {{$product->priceTotal}} ₽
            @else
        ...
        <p class="add__item">=</p>
            @if($isSale)
                <div class="add__item-box">
                    <p class="add__item red">{{ $product->priceTotal*$qtu }} ₽</p>
                    <p class="add__item small">(с учетом скидки)</p>
                </div>
            @else
                <p class="add__item orange">{{$product->price*$qtu }} ₽</p>
            @endif
        @endif
    @else
        <a wire:click.prevent="add('{{ $product->slug }}')" href="#" class="button">
            <svg class="icon">
        ...
        @if($isProduct)
        <p class="add__item rotate">+</p>
        <p class="add__item">{{$product->price}} ₽</p>
        <p class="add__item">=</p>
        <p class="add__item orange">{{$product->price}} ₽</p>
        @endif
    @endif
</div>
